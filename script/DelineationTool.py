...
if ";" not in input_raster_locations:
    ...
    # Store coordinate reference system of the first raster to change all of them in same CRS
    first_raster = arcpy.Raster(split_raster[0])
    first_raster_CRS = arcpy.Describe(first_raster).spatialReference.factoryCode
    
    # Mosaic To New Raster arguments
    raster_location = "..\\data\\"
...
# add a new field which will assign unique integers to each outlet if user does not wants to aggregate
# if user wants to aggregate watersheds into 1, the field will assign same unique integer to each row
...
arcpy.AddMessage("Converting watershed raster to polygon shapefile...")
